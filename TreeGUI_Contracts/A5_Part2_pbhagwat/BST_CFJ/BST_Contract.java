...
class BST_Iterator {
	...
	@Ensures({"root.ordered() || stack.peek().value==root.min()"})
	...
	@Ensures({ "old(count) > 0 || (stack.isEmpty() || result <= (stack.peek().value))" , "old(count) == 0 || old(count) == count + 1 " })
	public int next() {
		if (count == 0) {
			Tree node = stack.pop();
			value = node.value;
	...
	@Ensures({"node==null||((node!=null && old(node).min()==stack.peek().value))"})
